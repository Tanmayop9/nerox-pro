...
try {
    ...
    console.error('Failed to load lava.json, using fallback configuration:', error.message);
    lavaConfig = {
        nodes: [
            {
                name: 'primary-node',
...
const sortedNodes = lavaConfig.nodes.sort((a, b) => (a.priority || 1) - (b.priority || 1));
...
export class Manager {
    static { this.init = (client) => {
        ...
        manager.shoukaku.on('error', (name, error) => {
            client.log(`Lavalink node ${name} error: ${JSON.stringify(error)}`, 'error');
        });
        ...
        manager.shoukaku.on('disconnect', (name, reason) => {
            ...
            const availableNodes = [...manager.shoukaku.nodes.values()].filter(n => n.state === 2);
