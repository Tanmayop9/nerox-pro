...
export default {
  ...
  async execute(client, message, args) {
    ...
    if (
      supportCommands.includes(subcommand) &&
      !isOwner &&
      !isAdmin &&
      !isSupportRole
    ) {
      return message.reply({
        embeds: [
          client
            .embed(client.colors.error)
            .setDescription(
              "❌ You need to be a support staff member to use this command!",
            ),
        ],
      });
    }
    ...
    if (ticketParticipantCommands.includes(subcommand)) {
      ...
      const isParticipant =
        ticketData.participants?.includes(message.author.id) ||
...
async function showHelp(client, message, isAdmin, isSupportRole) {
  ...
  embed
    .setFooter({
      text: "NeroX Support Manager • Create tickets from the panel!",
    })
    .setTimestamp();
  ...
  collector.on("collect", async (i) => {
    ...
    switch (i.customId) {
      case "ticket_setup_category":
        await i.reply({
          content:
            "📁 Please mention the **category channel** where tickets will be created (e.g., mention or send category ID):",
          ephemeral: true,
        });
      ...
      case "ticket_setup_role":
        await i.reply({
          content:
            "👥 Please mention the **support role** that can view all tickets:",
          ephemeral: true,
        });
...
async function waitForCategorySetup(client, message, guildId, field) {
  ...
  await response.reply({
    embeds: [
      client
        .embed(client.colors.success)
        .setDescription(`✅ Category set to **${category.name}**!`),
    ],
  });
}
...
async function waitForRoleSetup(client, message, guildId) {
  ...
  const role =
    ...
    message.guild.roles.cache.get(response.content);
  ...
  if (!role) {
    return response.reply({
      embeds: [
        client
          .embed(client.colors.error)
          .setDescription("❌ Invalid role! Please mention a valid role."),
      ],
    });
  }
  ...
  await response.reply({
    embeds: [
      client
        .embed(client.colors.success)
        .setDescription(`✅ Support role set to ${role}!`),
    ],
  });
}
...
async function testSetup(client, interaction, guildId) {
  ...
  await interaction.reply({
    embeds: [
      client
        .embed(allGood ? client.colors.success : client.colors.warning)
        .setAuthor({
...
async function manageCategories(client, message, args) {
  ...
  switch (action) {
      ...
      if (
        ...
        return message.reply({
          embeds: [
            client
              .embed(client.colors.error)
              .setDescription("❌ A category with this name already exists!"),
          ],
        });
      }
    ...
    case "remove": {
      ...
      const index = config.ticketCategories.findIndex(
        (c) => c.name.toLowerCase() === name.toLowerCase(),
      );
      if (index === -1) {
        return message.reply({
      ...
      await message.reply({
        embeds: [
          client
            .embed(client.colors.success)
            .setDescription(`✅ Category **${removed.name}** removed!`),
        ],
      });
      break;
    }
    ...
    case "list": {
      ...
      const embed = client
        ...
        .setFooter({
          text: `Total: ${config.ticketCategories.length} categories`,
        });
    ...
    case "edit": {
        ...
        return message.reply({
          embeds: [
            client
              .embed(client.colors.error)
              .setDescription(
                "❌ Usage: `ticket category edit <name> <emoji/description> <value>`",
              ),
          ],
        });
      }

      const category = config.ticketCategories.find(
        (c) => c.name.toLowerCase() === name.toLowerCase(),
      );
      if (!category) {
        return message.reply({
        ...
        return message.reply({
          embeds: [
            client
              .embed(client.colors.error)
              .setDescription(
                "❌ Invalid field! Use: emoji, description, or name",
              ),
          ],
        });
      }
      ...
      await message.reply({
        embeds: [
          client
            .embed(client.colors.success)
            .setDescription(`✅ Updated **${name}** ${field} to: ${value}`),
        ],
      });
      break;
    }
...
async function createTicket(client, message, args) {
    ...
    return message.reply({
      embeds: [
        client
          .embed(client.colors.error)
          .setDescription(
            "❌ Ticket system is not configured! Ask an admin to run `ticket setup`.",
          ),
      ],
    });
  }
  ...
  const existingTicket = await findUserOpenTicket(
    client,
    guildId,
    message.author.id,
  );
    ...
    return message.reply({
      embeds: [
        client
          ...
          .setDescription(
            `⚠️ You already have an open ticket: <#${existingTicket.channelId}>\n\nPlease use that ticket or close it first.`,
          ),
      ],
    });
  }
  ...
  if (!category) {
    return message.reply({
      embeds: [
        client
          .embed(client.colors.error)
          .setDescription("❌ Ticket category not found! Please reconfigure."),
      ],
    });
  }
  ...
  if (categoryName && config.ticketCategories?.length > 0) {
    ticketCategory = config.ticketCategories.find(
      (c) =>
  ...
  const reason =
    args.slice(ticketCategory ? 1 : 0).join(" ") || "No reason provided";
  ...
  if (!target) {
    return message.reply({
      embeds: [
        client
          .embed(client.colors.error)
          .setDescription("❌ Please mention a user to add!"),
      ],
    });
  }
...
async function removeUserFromTicket(client, message, args) {
  ...
  ticketData.participants = ticketData.participants.filter(
    (id) => id !== target.id,
  );
  await client.db.tickets.set(message.channel.id, ticketData);
...
async function listTickets(client, message, args) {
  ...
  if (filtered.length === 0) {
    return message.reply({
      embeds: [
        client
          .embed(client.colors.info)
          .setDescription("📋 No tickets found with the specified filter."),
      ],
    });
  }
  ...
  const embed = client
    ...
    .setDescription(
      filtered
        ...
        .map((t) => {
          ...
          return (
            `${statusEmoji} **#${t.number.toString().padStart(4, "0")}** - ${channel ? channel : "Deleted"}\n` +
    ...
    .setFooter({
      text: `Total: ${filtered.length} tickets | Filter: ${filter}`,
    });
...
async function setLogChannel(client, message, args) {
  ...
  await message.reply({
    embeds: [
      client
        .embed(client.colors.success)
        .setDescription(`✅ Ticket log channel set to ${channel}!`),
    ],
  });
}
    ...
    if (i.customId === "ticket_reset_confirm") {
      ...
      await i.update({
        embeds: [
          client
            .embed(client.colors.success)
            .setDescription("✅ Ticket configuration has been reset!"),
        ],
        components: [],
      });
    } else {
...
async function logTicketAction(
  ...
  try {
    ...
    const embed = client
      .embed(colors[action] || client.colors.info)
      .setAuthor({
      ...
      .setDescription(
        `**Ticket:** #${ticketData.number.toString().padStart(4, "0")}\n` +
          ...
          (ticketData.closedAt
            ? `**Duration:** ${formatDuration(ticketData.closedAt - ticketData.createdAt)}`
            : ""),
      )
