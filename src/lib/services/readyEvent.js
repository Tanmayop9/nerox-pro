...
const checkPremiumExpiries = async (client) => {
  ...
  if (expiredUsers)
    client.log(`Removed ${expiredUsers} expired user premiums.`, "info");
  if (expiredServers)
    client.log(`Removed ${expiredServers} expired server premiums.`, "info");
};
...
const checkNoPrefixExpiries = async (client) => {
  ...
  if (expiredUsers)
    client.log(`Removed ${expiredUsers} expired no-prefix users.`, "info");
};
...
export const readyEvent = async (client) => {
  ...
  const userCount = {
    ...
    total: client.guilds.cache.reduce(
  ...
  client.log(
    `Ready in ${guildCount} guilds with ${userCount.total} users (${userCount.cached} cached).`,
    "info",
  );
  ...
  client.manager.shoukaku.once(
    "ready",
    async () => await load247Players(client),
  );
};
