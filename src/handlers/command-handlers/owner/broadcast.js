...
export default class Broadcast extends Command {
    constructor() {
        ...
        this.execute = async (client, ctx, args) => {
            if (!args.length) {
                await ctx.reply({
                    embeds: [client.embed().desc(`${client.emoji.cross} Please provide a message to broadcast.`)]
                });
                return;
            }
            ...
            const filter = m => m.author.id === ctx.author.id && ['confirm', 'cancel'].includes(m.content.toLowerCase());
            const collected = await ctx.channel.awaitMessages({ filter, max: 1, time: 30000 });

            if (!collected.size || collected.first().content.toLowerCase() === 'cancel') {
                await ctx.reply({
                    embeds: [client.embed().desc(`${client.emoji.cross} Broadcast cancelled.`)]
                });
                return;
            }
            ...
            const statusMsg = await ctx.reply({
                embeds: [client.embed().desc(`${client.emoji.timer} Broadcasting to ${guilds.size} servers...`)]
            });
