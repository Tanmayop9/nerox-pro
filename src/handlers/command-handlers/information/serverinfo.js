...
import { filter } from '../../core/helpers/filter.js';
...
export default class ServerInfo extends Command {
  ...
  async execute(client, ctx) {
    ...
    if (!guild) return ctx.reply({ content: 'This command must be used in a server.', ephemeral: true });
    ...
    const menu = new StringSelectMenuBuilder()
      ...
      .addOptions([
        ...
        { label: 'Security & Boosts', value: 'security', emoji: client.emoji.info }
      ]);
    ...
    collector.on('collect', async interaction => {
      ...
      if (val === 'overview') {
        ...
        const embed = client.embed()
          ...
          .desc(
            roles.length ? roles.slice(0, 30).join(', ') + (roles.length > 30 ? ` and ${roles.length - 30} more...` : '')
                        : 'No roles found.'
          )
