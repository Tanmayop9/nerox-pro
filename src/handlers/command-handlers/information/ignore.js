    ...
    execute = async (client, ctx, args) => {
        ...
        if (args[0].toLowerCase() === 'list') {
            ...
            if (!ignoredChannels.length) {
                return ctx.reply({
                    embeds: [client.embed().desc(`${client.emoji.cross} No channels are ignored.`)],
                });
            }
        ...
        if (args[0].toLowerCase() === 'add') {
            if (isIgnored) {
                return ctx.reply({
                    embeds: [client.embed().desc(`${client.emoji.cross} This channel is already ignored.`)],
                });
            }
        ...
        if (args[0].toLowerCase() === 'remove') {
            if (!isIgnored) {
                return ctx.reply({
                    embeds: [client.embed().desc(`${client.emoji.cross} This channel is not ignored.`)],
                });
            }
