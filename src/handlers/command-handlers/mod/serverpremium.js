    ...
    constructor() {
        ...
        this.execute = async (client, ctx, args) => {
            ...
            if (action === 'list') {
                ...
                if (!keys.length) {
                    return ctx.reply({
                        embeds: [client.embed().desc(`${cross} No servers currently have premium access.`)],
                    });
                }
                ...
                const serverDetails = await Promise.all(
                    keys.map(async (id, index) => {
                        ...
                        const expires = data?.expires ? `<t:${Math.floor(data.expires / 1000)}:R>` : '`∞`';
            ...
            if (!serverId) {
                return ctx.reply({
                    embeds: [client.embed().desc(`${cross} Please provide a valid server ID or use this command in a server.`)],
                });
            }
            ...
            if (action === 'add') {
                if (isPremium) {
                    return ctx.reply({
                        embeds: [client.embed().desc(`${cross} This server already has premium access.`)],
                    });
                }
            ...
            if (action === 'remove') {
                if (!isPremium) {
                    return ctx.reply({
                        embeds: [client.embed().desc(`${cross} This server is not in the premium list.`)],
                    });
                }
                ...
                return ctx.reply({
                    embeds: [
                        client.embed()
                            ...
                            .desc(`${check} Server \`${serverId}\` has been removed from premium.`),
                    ],
                });
            }
        };
