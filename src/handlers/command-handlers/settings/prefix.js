...
export default class Prefix extends Command {
    ...
    execute = async (client, ctx, args) => {
        if (!args.length) {
            const currentPrefix = await client.db.prefix.get(ctx.guild.id) || client.prefix;
            await ctx.reply({
                embeds: [
                    client
                        .embed()
        ...
        if (newPrefix.length > 2) {
            await ctx.reply({
                embeds: [
                    client
                        .embed()
                        .desc(`${client.emoji.cross} Prefix must be maximum 2 characters long!`),
                ],
            });
            return;
        }
        ...
        const emojiRegex = /[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1FA70}-\u{1FAFF}]|<a?:\w+:\d+>/gu;
